...
class ProductCategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        if (preg_match('/^F\d{3,}$/', $search)) {
            $category->where('id', 'like', (int)substr($search, 1), 'or');
        }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('product-category.get')->with('success', 'Data berhasil ditambahkan!');
    }
    ...
    {
        if (!preg_match('/^F\d+$/', $id)) return abort(404); // Pastikan format ID diawali "U"
        $product_category_id = (int)substr($id, 1); // Ambil angka setelah "U"
        $shop_id = Auth::user()->shop_id;
    ...
    {
        if (!preg_match('/^F\d+$/', $id)) return abort(404);
        $product_category_id = (int)substr($id, 1);
        $shop_id = Auth::user()->shop_id;
    ...
    public function destroy(string $id)
    {
        ...
        $productCategory_id = (int)substr($id, 1);
        $shop_id = Auth::user()->shop_id;

        ProductCategory::where('id', $productCategory_id)
            ->where('shop_id', $shop_id)
            ->delete();

        return redirect()->route('product-category.get')->with('success', 'Data berhasil dihapus!');
    }
