...
export default function Pagination({page = 1, totalPage = 1}: PropsWithChildren<{page?: number, totalPage?: number}>) {
    ...
    return (
        ...
        <ul className="flex items-center -space-x-px h-10 text-sm">
            <li>
            <button onClick={() => handlePagination(page - 1)} disabled={page === 1} className="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700">
            ...
            {
              paginationRange(totalPage, page)?.map((pagination, index) => (
                <li key={index}>
                  <button onClick={() => handlePagination(Number(pagination))} className={`flex items-center justify-center px-4 h-10 font-medium leading-tight border ${page === pagination ? 'text-white border-primary-500 bg-primary-500 hover:bg-primary-600 hover:border-primary-600' : 'text-gray-500 bg-white border-gray-300 hover:bg-primary-500 hover:text-white hover:border-primary-500'}`} disabled={page === pagination || pagination === '...'}>{pagination}</button>
                </li>
              ))
            }
            <li>
            <button onClick={() => handlePagination(page + 1)} disabled={page === totalPage} className="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700">
